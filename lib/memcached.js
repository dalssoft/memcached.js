	...
	this.process = function (data, callback) {
		...
		{	
			...
			if (!mdata.commandName) {
				mdata.commandName = commandName = cmd.MemcachedCommand.tryReadCommand(mdata);
			}
			...
			log.write(function(){ return "MemcachedTCPDataHandler.process: (command processed) command: " + commandName}, 2);
			...
			if (command.rawCommandLength != mdata.data.length) {
				try{
					log.write(function(){ return "MemcachedTCPDataHandler.process: (discard processed data) last command: " + commandName}, 2);
					var b = new Buffer(mdata.data.length - command.rawCommandLength);
					//var commandLength = command.rawCommandLength;
					//if (mdata.data.length < command.rawCommandLength) commandLength = mdata.data.length;
					...
					console.log("command.rawCommandLength: " + util.inspect(command.rawCommandLength));
