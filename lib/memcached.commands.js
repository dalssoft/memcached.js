...
var MemcachedCommand = function () {};
	...
	MemcachedCommand.tryParseStorageCommand = function (mdata) {
		...
		var end_token = '\r\n';
		var data = mdata.data.toString();
		var endCommandLine = data.indexOf(end_token);
		var commandLine = data.substring(0, endCommandLine);
		var tmpParams = commandLine.split(" ");
		var bytes = parseInt(tmpParams[4]);
		if (bytes < (data.length - endCommandLine + end_token.length)) {
			var commandDataStr = data.substr(endCommandLine + end_token.length, bytes);
			...
			log.write(function(){ 
				...
				+ " command:" + result.command + " key:" + result.key + " bytes:" + result.bytes;
			}, 2);
		}
